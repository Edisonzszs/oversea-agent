...
import { getFieldsForStep, type ExtractField, type ParsedCandidate } from "./fieldCatalog";
...
export function buildChatSystemPrompt(step: number, mode: Mode | null): string {
  ...
  return [
    "你是「小海」，企业境外投资(ODI)合规自查的伴填助手。你要同时胜任两件事：",
    ...
    "",
    "【可抽取字段】（仅限当前步骤；value 必须是允许值之一并使用 code；不确定的字段整字段省略，严禁臆测；evidence 为用户原文摘录；confidence 为 0-1）：",
    fieldLines,
    "",
...
export function parseChatResponse(content: string, fields: ExtractField[]): ChatResponse {
  ...
  for (const c of Array.isArray(obj?.candidates) ? obj.candidates : []) {
    ...
    if (f.kind === "multi") {
      ...
      const valid = f.allowed ? codes.filter(x => f.allowed!.some(a => a.value === x)) : codes;
      if (valid.length === 0) continue;
      candidates.push({ field: f, value: valid.join(","), confidence: conf, evidence, lowConf: conf < 0.8 });
    } else {
      if (f.kind === "select" && f.allowed && !f.allowed.some(a => a.value === raw)) continue;
      candidates.push({ field: f, value: raw, confidence: conf, evidence, lowConf: conf < 0.8 });
    }
  }
